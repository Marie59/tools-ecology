...
df.full$QEBM.2 <- ifelse(df.full$QEBM.2 %in% c("-Inf", "NaN"), NA, df.full$QEBM.2)
  
df1 <- df %>% filter(Site==v)
df1 <- droplevels(df1)
...
qecb.scale <- data.frame(matrix(NA, nrow = 5, ncol = 3))
colnames(qecb.scale) <- c("Classe de l'indice", "Limites équidistantes",  "Signification")

qecb.scale$`Classe de l'indice` <- c("1", "2", "3", "4", "5")
qecb.scale$`Limites équidistantes` <- c("-360 ≤ QECB < -216", "-216 ≤ QECB < -72", "-72 ≤ QECB < +72", "+72 ≤ QECB < +216", "+216 ≤ QECB = +360")
qecb.scale$Signification <- c("Très mauvais état écologique", "Mauvais état écologique", "État écologique moyen", "Bon état écologique", "Très bon état écologique")
...
#library(kableExtra) # kableExtra doesn't work for word output
#qecb.scale %>%
#  kbl() %>%
#  kable_classic(full_width = T, html_font = "Cambria") #%>%
  #row_spec(1, row = 1, background = "red") %>%
  #row_spec(2, row = 2, background = "orange") %>%
  #row_spec(3, row = 3, background = "yellow") %>%
  #row_spec(4, row = 4, background = "green") %>%
  #row_spec(5, row = 5, background = "blue")
...
filter(df.full, QEBM.2 >= -360) -> dt.
filter(dt., QEBM.2 <= 360) -> dt. 
filter(dt., QEBM.2 >= quantile(dt.$QEBM.2, c(0.05), na.rm=T)) -> dt.bis
filter(dt.bis, QEBM.2 <= quantile(dt.bis$QEBM.2, c(0.95), na.rm=T)) -> dt.bis

one <- round(mean(unlist(filter(dt.bis, QEBM.2 <= quantile(dt.bis$QEBM.2, 0.25, na.rm = T))["QEBM.2"])), digits = 0)
two <- round(mean(unlist(filter(dt.bis, QEBM.2 > quantile(dt.bis$QEBM.2, 0.25, na.rm = T) & QEBM.2 <= quantile(dt.bis$QEBM.2, 0.5, na.rm = T))["QEBM.2"])), digits = 0)
three <- round(mean(unlist(filter(dt.bis, QEBM.2 > quantile(dt.bis$QEBM.2, 0.5, na.rm = T) & QEBM.2 <= quantile(dt.bis$QEBM.2, 0.75, na.rm = T))["QEBM.2"])), digits = 0)
four <- round(mean(unlist(filter(dt.bis, QEBM.2 > quantile(dt.bis$QEBM.2, 0.75, na.rm = T))["QEBM.2"])), digits = 0)
...
qecb.scale <- data.frame(matrix(NA, nrow = 5, ncol = 3))
colnames(qecb.scale) <- c("Classe de l'indice", "Limites quantiles",  "Signification")

qecb.scale$`Classe de l'indice` <- c("1", "2", "3", "4", "5")
qecb.scale$`Limites quantiles` <- c(
...
#  row_spec(2, row = 2, background = "orange") %>%
#  row_spec(3, row = 3, background = "yellow") %>%
#  row_spec(4, row = 4, background = "olivedrab") %>%
#  row_spec(5, row = 5, background = "blue")

library(flextable)
qecb.scale %>%
  flextable() -> qu.tbl
qu.tbl <- bg(qu.tbl, i = 1, bg = "red")
qu.tbl <- bg(qu.tbl, i = 2, bg = "orange")
qu.tbl <- bg(qu.tbl, i = 3, bg = "yellow")
qu.tbl <- bg(qu.tbl, i = 4, bg = "olivedrab")
qu.tbl <- bg(qu.tbl, i = 5, bg = "blue")
...
txt. <- data.frame(matrix(NA, nrow = 2, ncol = 1))
colnames(txt.) <- "res."
txt.[1,1] <- ifelse(min(df1$qecb.min, na.rm = T) < -360, " (le minimum est en dehors des bornes théoriques)", "")
txt.[2,1] <- ifelse(max(df1$qecb.max, na.rm = T) > 360, " (le maximum est en dehors des bornes théoriques)", "")

```

Les valeurs médianes du QECB au site `r unique(df1$Site.bis)` (points noirs) sont comprises entre un minimum égal à `r round(min(df1$qecb.med, na.rm = T), digits = 0)` et un maximum égal à `r round(max(df1$qecb.med, na.rm = T), digits = 0)`. Les valeurs individuelles, par bloc mobile (barres), sont quant à elles comprises entre un minimum égal à `r round(min(df1$qecb.min, na.rm = T), digits = 0)``r txt.[1,1]` et un maximum égal à `r round(max(df1$qecb.max, na.rm = T), digits = 0)``r txt.[2,1]`. Ces valeurs propres au site `r unique(df1$Site.bis)` peuvent être resituées par rapport aux autres valeurs médianes du QECB obtenues pour les vingt-trois autres sites suivis sur la période 2014-`r ifelse(max(df1$Annee) <= 2017, 2017, max(df1$Annee))` à l’échelle de la façade Manche-Atlantique française (points gris). Tous sites confondus, les valeurs médianes de cet indice occupent une fenêtre plus restreinte que celle entre leurs extrema théoriques, centrées sur la valeur de 6.
...
par(mar=c(5, 5, 4, 2) + 0.1)
  ...
  plot(df$Date, df$qecb.med, xlim = c(xmin., xmax.), ylim = c(-360, 360), pch = 19, cex = 1, main = unique(df1$Site.bis), xlab = "Année", #ylab = expression(paste("", QEBM^{2},"")), 
       ylab = "QECB", col = "grey")
...
par(mar=c(5, 5, 4, 2) + 0.1)
  ...
  #min(df$qecb.med, na.rm = T)
  #max(df$qecb.med, na.rm = T)
  #head(sort(df$qecb.med))
  #tail(sort(df$qecb.med))
  
  ymin. = ifelse(min(df1$qecb.med, na.rm = T) < -70, -360, -70)
  ymax. = ifelse(max(df1$qecb.med, na.rm = T) > 200, 360, 200)
  
  plot(df$Date, df$qecb.med, xlim = c(xmin., xmax.), ylim = c(ymin., ymax.), pch = 19, main = "", xlab = "", ylab = "", type = 'n', axes = F)
  ...
  par(new = T)
  plot(df$Date, df$qecb.med, xlim = c(xmin., xmax.), ylim = c(ymin., ymax.), pch = 19, cex = 1, main = unique(df1$Site.bis), xlab = "Année", #ylab = expression(paste("", QEBM^{2},"")), 
       ylab = "QECB", col = "grey")
  points(df1$Date, df1$qecb.med, pch = 19, cex = 1.5)
  arrows(df1$Date, df1$qecb.med, df1$Date, df1$qecb.max, code = 3, angle = 90, length = 0.00)
  arrows(df1$Date, df1$qecb.med, df1$Date, df1$qecb.min, code = 3, angle = 90, length = 0.00)
...
Ce bref rapport présente l'évolution de la valeur de l'indice QECB au site `r unique(df1$Site.bis)`, suivi de `r min(unique(df1$Annee), na.rm = T)` à `r max(unique(df1$Annee), na.rm = T)`, et la resitue par rapport aux valeurs QECB des vingt-trois autres champs de blocs suivis  pour l'essentiel de 2014 à 2017 sur la façade Manche-Atlantique française. Il illustre l'évolution dans le temps de l'impact de la pression de la pêche à pied et du retournement des blocs sur la qualité écologique de l'habitat.
